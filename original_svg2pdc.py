...
def sum_points(p1, p2):
    print("sum_points: p1: {}, p2: {}".format(p1, p2))
    return p1[0] + p2[0], p1[1] + p2[1]


def subtract_points(p1, p2):
    print("subtract_points: p1: {}, p2: {}".format(p1, p2))
    return p1[0] - p2[0], p1[1] - p2[1]


def round_point(p):
    print("round_point: p: {}".format(p))
...
def convert_to_pebble_coordinates(point, precise=False):
    ...
    # translate point by (-0.5, -0.5)
    translated = sum_points(point, (-0.5, -0.5))
    if precise:
        # scale point for precise coordinates
        translated = scale_point(translated, 8)
...
def compare_points(p1, p2):
    print("compare_points: p1: {}, p2: {}".format(p1, p2))
    return p1[0] == p2[0] and p1[1] == p2[1]


class InvalidPointException(Exception):
    # print("InvalidPointException: {}".format(Exception))
    pass
    ...
    def __init__(self, points, translate, stroke_width=0, stroke_color=0, fill_color=0, precise=False,
        ...
        for i in range(len(points)):
            points[i], valid = convert_to_pebble_coordinates(
                sum_points(points[i], translate), precise)
...
class PathCommand(Command):
    def __init__(self, points, path_open, translate, stroke_width=0, stroke_color=0, fill_color=0, precise=False,
        ...
        Command.__init__(self, points, translate, stroke_width,
                         stroke_color, fill_color, precise, raise_error)
...
class CircleCommand(Command):
    def __init__(self, center, radius, translate, stroke_width=0, stroke_color=0, fill_color=0):
        ...
        Command.__init__(self, points, translate,
...
def get_viewbox(root):
    print("get_viewbox: root: {}".format(root))
    try:
...
def get_translate(group):
    print("get_translate: group: {}".format(group))
...
def convert_color(rgb, a, truncate=True):
    print("convert_color: rgb: {}, a: {}, truncate: {}".format(rgb, a, truncate))
...
def parse_color(color, opacity, truncate):
    print("parse_color: color: {}, opacity: {}, truncate: {}".format(
        color, opacity, truncate))
...
def calc_opacity(a1, a2):
    print("calc_opacity: a1: {}, a2: {}".format(a1, a2))
    try:
        a1 = float(a1)
    except (ValueError, TypeError):
        a1 = 1.0
...
def get_points_from_str(point_str):
    print("get_points_from_str: point_str: {}".format(point_str))
    points = []
...
def parse_path(element, translate, stroke_width, stroke_color, fill_color, precise, raise_error):
    print("parse_path: element: {}, translate: {}, stroke_width: {}, stroke_color: {}, fill_color: {}, precise: {}, raise_error: {}".format(
        element, translate, stroke_width, stroke_color, fill_color, precise, raise_error))
    import svg.path
    d = element.get('d')
    if d is not None:
...
def parse_circle(element, translate, stroke_width, stroke_color, fill_color, precise, raise_error):
    print("parse_circle: element: {}, translate: {}, stroke_width: {}, stroke_color: {}, fill_color: {}, precise: {}, raise_error: {}".format(
        element, translate, stroke_width, stroke_color, fill_color, precise, raise_error))
    cx = element.get('cx')      # center x-value
    cy = element.get('cy')      # center y-value
    radius = element.get('r')   # radius
    if radius is None:
        # 'z' sometimes used instead of 'r' for radius
        radius = element.get('z')
...
def parse_polyline(element, translate, stroke_width, stroke_color, fill_color, precise, raise_error):
    print("parse_polyline: element: {}, translate: {}, stroke_width: {}, stroke_color: {}, fill_color: {}, precise: {}, raise_error: {}".format(
        element, translate, stroke_width, stroke_color, fill_color, precise, raise_error))
...
def parse_polygon(element, translate, stroke_width, stroke_color, fill_color, precise, raise_error):
    print("parse_polygon: element: {}, translate: {}, stroke_width: {}, stroke_color: {}, fill_color: {}, precise: {}, raise_error: {}".format(
        element, translate, stroke_width, stroke_color, fill_color, precise, raise_error))
    points = get_points_from_str(element.get('points'))
    if not points:
        return None
...
def parse_line(element, translate, stroke_width, stroke_color, fill_color, precise, raise_error):
    print("parse_line: element: {}, translate: {}, stroke_width: {}, stroke_color: {}, fill_color: {}, precise: {}, raise_error: {}".format(
        element, translate, stroke_width, stroke_color, fill_color, precise, raise_error))
    try:
...
def parse_rect(element, translate, stroke_width, stroke_color, fill_color, precise, raise_error):
    print("parse_rect: element: {}, translate: {}, stroke_width: {}, stroke_color: {}, fill_color: {}, precise: {}, raise_error: {}".format(
        element, translate, stroke_width, stroke_color, fill_color, precise, raise_error))
    try:
...
def create_command(translate, element, precise=False, raise_error=False, truncate_color=True):
    print("create_command: translate: {}, element: {}, precise: {}, raise_error: {}, truncate_color: {}".format(
        translate, element, precise, raise_error, truncate_color))
    ...
    print("CALLING PARSE COLOR")
    stroke_color = parse_color(stroke, calc_opacity(
        stroke_opacity, opacity), truncate_color)
    print(stroke_color)
    print("CALLING PARSE COLOR")
    fill_color = parse_color(fill, calc_opacity(
        fill_opacity, opacity), truncate_color)
    print(fill_color)
...
def get_commands(translate, group, precise=False, raise_error=False, truncate_color=True):
    print("get_commands: translate: {}, group: {}, precise: {}, raise_error: {}, truncate_color: {}".format(
        translate, group, precise, raise_error, truncate_color))
    ...
    for child in list(group):
        ...
        if tag == 'layer' or tag == 'g':
            ...
            if tag == 'g':
                group_opacity = child.get(
                    'opacity') if child.get('opacity') else None
                group_fill = child.get('fill') if child.get('fill') else None
                group_fill_opacity = child.get(
                    'fill-opacity') if child.get('fill-opacity') else None
                group_stroke = child.get(
                    'stroke') if child.get('stroke') else None
                group_stroke_opacity = child.get(
                    'stroke-opacity') if child.get('stroke-opacity') else None
                group_stroke_width = child.get(
                    'stroke-width') if child.get('stroke-width') else None
                # fix stroke-width for '1px' to be int 1
                if group_stroke_width:
                    group_stroke_width = int(
                        float([x for x in group_stroke_width if x in '0123456789.']))
                ...
                if 'translate' in transform:
                    translate_strs = re.search(
                        r'(?:translate\()(.*),(.*)\)', transform).group(1, 2)
                    translate = (
                        translate[0] + float(translate_strs[0]), translate[1] + float(translate_strs[1]))
            child_translate = get_translate(child)
            translate = (translate[0] + child_translate[0],
                         translate[1] + child_translate[1])
            cmd_list, err = get_commands(
                translate, child, precise, raise_error, truncate_color)
            commands += cmd_list
            if err:
                error = True
        else:
            try:
                ...
                if transform is not None and 'translate' in transform:
                    ...
                    child_translate = (
                        translate[0] + float(translate_strs[0]), translate[1] + float(translate_strs[1]))
                c = create_command(child_translate, child,
                                   precise, raise_error, truncate_color)
                if c is not None:
                    commands.append(c)
...
def get_xml(filename):
    print("get_xml: filename: {}".format(filename))
    try:
...
def serialize(commands):
    print("serialize: commands: {}".format(commands))
...
def print_commands(commands):
    print("print_commands: commands: {}".format(commands))
    for c in commands:
        print(str(c))


def print_frames(frames):
    print("print_frames: frames: {}".format(frames))
...
def serialize_frame(frame, duration):
    print("serialize_frame: frame: {}, duration: {}".format(frame, duration))
    return pack('H', duration) + serialize(frame)   # Frame duration


def pack_header(size):
    print("pack_header: size: {}".format(size))
...
def serialize_sequence(frames, size, duration, play_count):
    ...
    print("serialize_sequence: s: {}".format(s))
    output = "PDCS"
    output += pack('I', len(s))
    output += s
    return output
...
def serialize_image(commands, size):
    print("serialize_image: commands: {}, size: {}".format(commands, size))
    s = pack_header(size)
...
def parse_svg_image(filename, precise=False, raise_error=False):
    print("parse_svg_image: filename: {}".format(filename))
...
def parse_svg_sequence(dir_name, precise=False, raise_error=False):
    print("parse_svg_sequence: dir_name: {}".format(dir_name))
    frames = []
    error_files = []
    ...
    translate, size = get_info(get_xml(file_list[0]))
    for filename in file_list:
        cmd_list, error = get_commands(
            translate, get_xml(filename), precise, raise_error)
...
def create_pdc_from_path(path, sequence, out_path, verbose, duration, play_count, precise=False, raise_error=False):
    print("create_pdc_from_path: path: {}, sequence: {}, out_path: {}, verbose: {}, duration: {}, play_count: {}, precise: {}, raise_error: {}".format(
        path, sequence, out_path, verbose, duration, play_count, precise, raise_error))
    print("Creating PDC from " + path)
